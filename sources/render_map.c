...
/*   Updated: 2024/09/27 20:23:11 by marcsilv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_tile(t_game *game, int x, int y, char id)
{
	if (game->map->matrix[y][x] == '1')
		...
		game->window->win, game->img->box, x * 32, y * 32);
	else if (game->map->matrix[y][x] == '0')
		mlx_put_image_to_window(game->window->mlx, \
		game->window->win, game->img->center, x * 32, y * 32);
	else if (game->map->matrix[y][x] == 'C')
		mlx_put_image_to_window(game->window->mlx, \
		game->window->win, game->img->coin, x * 32, y * 32);
	else if (game->map->matrix[y][x] == 'E' && !game->map->is_map_exitable)
		mlx_put_image_to_window(game->window->mlx, \
		game->window->win, game->img->exit_closed, x * 32, y * 32);
	else if (game->map->matrix[y][x] == 'E' && game->map->is_map_exitable)
		mlx_put_image_to_window(game->window->mlx, \
		game->window->win, game->img->exit_open, x * 32, y * 32);
	else if (game->map->matrix[y][x] == 'P')
	{
		mlx_put_image_to_window(game->window->mlx, \
		game->window->win, game->img->pac, x * 32, y * 32);
		game->map->pac_x = x;
		game->map->pac_y = y;
	}
}
